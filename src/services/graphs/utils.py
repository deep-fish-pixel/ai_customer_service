...
async def getVideoHistoryAndNextChat(chat_message: str, last_chat_message: str, last_chat_message_answer: str):
  """
    根据用户的回答，进行合理回应，更拟人化
  ...
  llm = getChatOpenAI()
  prompt = ChatPromptTemplate.from_template("""
        你是一个跟用户进行友好沟通的机器人，用户需要你对当前的会话做为生成视频输入的美化和补充的处理，并做出合理回复。
        上一个会话: {last_chat_message}
        用户的回答: {last_chat_message_answer}
        当前会话：{chat_message}
