...
def delete_list_record_graph() -> StateGraph:
    """删除列表记录的属性值工作流，收集所有必要信息并完成数据库存储"""
    graph = StateGraph(AgentState)

    # 定义信息收集节点
    async def collect_record_type(state: AgentState):
        response = await getHistoryAndNextQuestion("请问您要删除的类型是？（日程会议/请假申请/预定机票/预定酒店）", state['history'][-1], state['query'])

        return {** state, "query": response.content, "task_status": 1}

    async def collect_index(state: AgentState):
        response = await getHistoryAndNextQuestion("请问您要删除的是第几条？", state['history'][-1], state['query'])

        return {** state, "query": response.content, "task_status": 1}

    async def collect_id(state: AgentState):
        response = await getHistoryAndNextQuestion("请问您要删除的id是？", state['history'][-1], state['query'])
    ...
    async def extract_info(state: AgentState) -> AgentState:
        ...
        update_info = DeleteInfo(**task_collected)
        user_response = [*state["history"], state["query"]]

        # 创建LLM提示模板，用于解析和验证用户输入
        prompt = ChatPromptTemplate.from_template("""
        你是一个信息提取助手，需要从用户的回答中提取删除列表第n个记录的某个字段名称所需的信息。
        请根据用户当前的回答，提取相关信息并以JSON格式返回。
        当前已收集的信息: {existing_info}
        用户的回答: {user_response}
        需要提取的字段包括id(id，需要根据列表的index获取到)，record_type(记录类型(日程会议/请假申请/预定机票/预定酒店)，不需要包含(记录/信息)2个字)，index(序号，默认从1开始，如果记录只有一条，默认1，对应数据的下标为index-1)，size(列表的总数)。
        ...
        # 初始化LLM和解析器
        llm = getChatOpenAI()
        parser = JsonOutputParser(pydantic_object=DeleteInfo)
    ...
    def should_continue(state: AgentState) -> str:
        update_info = DeleteInfo(**state.get("task_collected", {}))
    ...
    async def save_to_database(state: AgentState):
        ...
        try:
            ...
            result = relative_db_service.delete_table_record(
